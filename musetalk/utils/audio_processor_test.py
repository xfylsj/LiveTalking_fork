...
class AudioProcessor:
    ...
    def get_audio_stream_feature_origin(self, audio_stream_data, start_index=0, weight_dtype=None):
        """
        处理音频流数据并提取特征
    ...
    def get_audio_stream_feature_new(self, audio_stream_data, start_index=0, weight_dtype=None):
        """
        处理音频流数据并提取特征
...
def test_silence():
    ...
    # 将特征输入 get_whisper_chunk，使用完整参数
    ...
    print("whisper_chunks type:", type(whisper_chunks))
    print("whisper_chunks shape:", whisper_chunks.shape if hasattr(whisper_chunks, 'shape') else "No shape")
    print("whisper_chunks:", whisper_chunks)
